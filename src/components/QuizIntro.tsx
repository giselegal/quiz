...
export default QuizIntro;};  );    </AutoFixedImages>      </div>        </form>          <Button type="submit">Iniciar Quiz</Button>          />            className="mb-4"            onChange={(e) => setNome(e.target.value)}            value={nome}            placeholder="Digite seu nome"            type="text"          <Input        >          onSubmit={handleSubmit}          className="mt-8 w-full max-w-md flex flex-col items-center"        <form        </div>          </picture>            />              }}                setTinyBase64('');                imageLoaded.current = true;              onLoad={() => {              }}                transition: 'filter 0.3s ease-in-out'                filter: tinyBase64 ? 'blur(20px)' : 'none',              style={{              className="w-full h-auto"              alt="Intro"              src={STATIC_INTRO_IMAGE_URLS.png}            <img            />              media="(min-width: 640px)"              type="image/webp"              srcSet={STATIC_INTRO_IMAGE_URLS.webp.small}            <source            />              media="(min-width: 768px)"              type="image/webp"              srcSet={STATIC_INTRO_IMAGE_URLS.webp.medium}            <source            />              media="(min-width: 1024px)"              type="image/webp"              srcSet={STATIC_INTRO_IMAGE_URLS.webp.large}            <source            />              media="(min-width: 640px)"              type="image/avif"              srcSet={STATIC_INTRO_IMAGE_URLS.avif.small}            <source            />              media="(min-width: 768px)"              type="image/avif"              srcSet={STATIC_INTRO_IMAGE_URLS.avif.medium}            <source            />              media="(min-width: 1024px)"              type="image/avif"              srcSet={STATIC_INTRO_IMAGE_URLS.avif.large}            <source          <picture>        <div className="main-image-container mt-8" ref={mainImageRef}>        </div>          </picture>            />              className="h-16"              alt="Logo"              src={STATIC_LOGO_IMAGE_URLS.png}            <img            <source srcSet={STATIC_LOGO_IMAGE_URLS.webp} type="image/webp" />            <source srcSet={STATIC_LOGO_IMAGE_URLS.avif} type="image/avif" />          <picture>        <div className="logo-container mt-8">      >        data-section="intro"        }}          minHeight: '100vh'          background: 'linear-gradient(180deg, #FFFFFF 0%, #FBF8F4 100%)',        style={{        className="quiz-intro flex flex-col items-center w-full"      <div     <AutoFixedImages>  return (  };    }      onStart(nome);    if (nome.trim()) {    e.preventDefault();  const handleSubmit = (e) => {  }, []);    loadTinyBase64();    };      }        console.error('[QuizIntro] Erro ao carregar imagem tiny:', error);      } catch (error) {        }          }            }              } catch (e) {}                sessionStorage.setItem('quiz_intro_tiny_base64', base64Data);              try {              setTinyBase64(base64Data);            if (base64Data) {            const base64Data = await loadTinyImageAsBase64(STATIC_INTRO_IMAGE_URLS.placeholder);          } else {            setTinyBase64(cachedImage);          if (cachedImage) {          const cachedImage = sessionStorage.getItem('quiz_intro_tiny_base64');        if (!tinyBase64 && !imageLoaded.current) {      try {    const loadTinyBase64 = async () => {  useEffect(() => {  }, []);    };      if (lcpCandidatePreload.parentNode) lcpCandidatePreload.parentNode.removeChild(lcpCandidatePreload);      if (dnsPrefetchLink.parentNode) dnsPrefetchLink.parentNode.removeChild(dnsPrefetchLink);      if (preconnectLink.parentNode) preconnectLink.parentNode.removeChild(preconnectLink);    return () => {        document.head.appendChild(lcpCandidatePreload);    lcpCandidatePreload.setAttribute('fetchpriority', 'high');    lcpCandidatePreload.type = 'image/avif';    lcpCandidatePreload.href = STATIC_INTRO_IMAGE_URLS.avif.large;    lcpCandidatePreload.as = 'image';    lcpCandidatePreload.rel = 'preload';    const lcpCandidatePreload = document.createElement('link');    document.head.appendChild(dnsPrefetchLink);    dnsPrefetchLink.href = 'https://res.cloudinary.com';    dnsPrefetchLink.rel = 'dns-prefetch';    const dnsPrefetchLink = document.createElement('link');    document.head.appendChild(preconnectLink);    preconnectLink.crossOrigin = 'anonymous';    preconnectLink.href = 'https://res.cloudinary.com';    preconnectLink.rel = 'preconnect';    const preconnectLink = document.createElement('link');  useEffect(() => {  }, []);    }      return () => clearTimeout(idleTimer);      }, 2000);        preloadCriticalImages('quiz');      const idleTimer = setTimeout(() => {    } else {      }, { timeout: 2000 });        preloadCriticalImages('quiz');      requestIdleCallback(() => {    if (typeof requestIdleCallback === 'function') {  useEffect(() => {  }, []);    }      };        window.removeEventListener('resize', updateWidth);      return () => {      window.addEventListener('resize', updateWidth);      updateWidth();      };        }          setMainImageWidth(mainImageRef.current.offsetWidth);        if (mainImageRef.current) {      const updateWidth = () => {    if (mainImageRef.current) {  useEffect(() => {    const imageLoaded = useRef(false);  const mainImageRef = useRef(null);    const [tinyBase64, setTinyBase64] = useState('');  const [mainImageWidth, setMainImageWidth] = useState(0);  const [nome, setNome] = useState('');}) => {  onStartexport const QuizIntro = ({}  onStart: (nome: string) => void;interface QuizIntroProps {// --- Fim das otimizações de escopo do módulo ---};  png: `${INTRO_IMAGE_BASE_URL}f_png,q_75,w_345,c_limit,fl_progressive/${INTRO_IMAGE_ID}.png`  placeholder: `${INTRO_IMAGE_BASE_URL}f_webp,q_5,w_30,c_limit,e_blur:100/${INTRO_IMAGE_ID}.webp`,  },    large: buildOptimizedIntroImageUrl(INTRO_IMAGE_BASE_URL, INTRO_IMAGE_ID, 'webp', 450, 80)    medium: buildOptimizedIntroImageUrl(INTRO_IMAGE_BASE_URL, INTRO_IMAGE_ID, 'webp', 400, 75),    small: buildOptimizedIntroImageUrl(INTRO_IMAGE_BASE_URL, INTRO_IMAGE_ID, 'webp', 345, 70),    tiny: buildTinyIntroImageUrl(INTRO_IMAGE_BASE_URL, INTRO_IMAGE_ID, 'webp', 200),  webp: {  },    large: buildOptimizedIntroImageUrl(INTRO_IMAGE_BASE_URL, INTRO_IMAGE_ID, 'avif', 450, 85)    medium: buildOptimizedIntroImageUrl(INTRO_IMAGE_BASE_URL, INTRO_IMAGE_ID, 'avif', 400, 80),    small: buildOptimizedIntroImageUrl(INTRO_IMAGE_BASE_URL, INTRO_IMAGE_ID, 'avif', 345, 75),    tiny: buildTinyIntroImageUrl(INTRO_IMAGE_BASE_URL, INTRO_IMAGE_ID, 'avif', 200),  avif: {const STATIC_INTRO_IMAGE_URLS = {};  avif: `${LOGO_BASE_URL}f_avif,q_auto,w_140,h_60,c_fit,dpr_auto,e_sharpen:100/${LOGO_IMAGE_ID}.avif`  png: `${LOGO_BASE_URL}f_png,q_auto,w_140,h_60,c_fit,dpr_auto,e_sharpen:100/${LOGO_IMAGE_ID}.png`,  webp: `${LOGO_BASE_URL}f_webp,q_auto,w_140,h_60,c_fit,dpr_auto,e_sharpen:100/${LOGO_IMAGE_ID}.webp`,const STATIC_LOGO_IMAGE_URLS = {};  return `${baseUrl}f_${format},q_60,w_${width},c_limit,dpr_1.0/${imageId}.${format}`;const buildTinyIntroImageUrl = (baseUrl, imageId, format, width) => {};  return `${baseUrl}f_${format},q_${quality},w_${width},c_limit,dpr_auto,fl_progressive,fl_lossy${width > 300 ? ',e_sharpen:30' : ''}/${imageId}.${format}`;const buildOptimizedIntroImageUrl = (baseUrl, imageId, format, width, quality) => {const INTRO_IMAGE_ID = "v1746838118/20250509_2137_Desordem_e_Reflex%C3%A3o_simple_compose_01jtvszf8sfaytz493z9f16rf2_z1c2up";const INTRO_IMAGE_BASE_URL = "https://res.cloudinary.com/dqljyf76t/image/upload/";const LOGO_IMAGE_ID = "v1744911572/LOGO_DA_MARCA_GISELE_r14oz2";const LOGO_BASE_URL = "https://res.cloudinary.com/dqljyf76t/image/upload/";// --- Otimizações: Constantes e funções movidas para o escopo do módulo ---} from '@/utils/inlineImageUtils';  getTinyImageUrl          getOptimizedImageUrl,   loadTinyImageAsBase64, import { import AutoFixedImages from './ui/AutoFixedImages';import { preloadCriticalImages } from '@/utils/imageManager';import { Input } from './ui/input';import { Button } from './ui/button';import { useState, useEffect, useRef } from 'react';import React from 'react';
